...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const QuizMundoPage(),
    );
  }
}

class QuizMundoPage extends StatefulWidget {
  const QuizMundoPage({super.key});

  @override
  // ignore: library_private_types_in_public_api
  _QuizMundoPageState createState() => _QuizMundoPageState();
}

class _QuizMundoPageState extends State<QuizMundoPage> {
  late SharedPreferences _prefsMundo;
  int _currentFaseIndexMundo = 0;
  int _currentQuestionIndexMundo = 0;
  int _pontosMundo = 0;
  int _scoreMundo = 0;
  late Timer _timer;
  int _secondsRemaining = 60;
  double _progressValueMundoAcerto = 0.0;
  double _progressValueMundoErro = 0.0; // Valor do progresso (0.0 a 1.0)
  ...
  Future<void> _initprefsMundo() async {
    _prefsMundo = await SharedPreferences.getInstance();
    _currentFaseIndexMundo = _prefsMundo.getInt('currentFaseIndexMundo') ?? 0;
    _pontosMundo = _prefsMundo.getInt('pontosMundo') ?? 0;
  ...
  void _barraPergunta() {
    int numero = (_currentFaseIndexMundo + 1);

    double resultadoDivisaoAcerto = 1.0 / (20 * numero.toDouble());
    if (_progressValueMundoAcerto < numero.toDouble()) {
      _progressValueMundoAcerto += resultadoDivisaoAcerto;
      _pontosMundo += 501;
  ...
  void _barraPerguntaErrada() {
    ...
    if (_progressValueMundoErro < 1.0) {
      _progressValueMundoErro += resultadoDivisao;
  ...
  void _handlePhaseCompletion() {
    setState(() {
      _scoreMundo = _pontosMundo;
      _pontosMundo = 0;
      _currentQuestionIndexMundo = 0;
      _progressValueMundoAcerto = 0.0;
      _progressValueMundoErro = 0.0;
      _resetTimer();
      _initprefsMundo(); //misturar perguntas
      _saveGameMundo();
    });
  ...
  void _handleAnswer(int selectedIndex) {
    if (_secondsRemaining == 0) {
      ...
      _currentQuestionIndexMundo = 0;
      _resetTimer();
      _initprefsMundo(); // Shuffle questions
      return;
    }
    ...
    final currentQuestion =
        quizFases[_currentFaseIndexMundo].perguntas[_currentQuestionIndexMundo];
  ...
  void _handleCorrectAnswer() {
    ...
    quizFases[_currentFaseIndexMundo]
        .perguntas
        .removeAt(_currentQuestionIndexMundo);
    quizFases[_currentFaseIndexMundo].perguntas.add(currentQuestion);
  ...
  void _handleIncorrectAnswer() {
    ...
    Future.delayed(const Duration(seconds: 4), () {
      final currentQuestion = quizFases[_currentFaseIndexMundo]
          .perguntas[_currentQuestionIndexMundo];
  ...
  void _advanceToNextQuestionOrPhase() {
    if (_pontosMundo < 9999 * (_currentFaseIndexMundo + 1)) {
      // Avança para a próxima pergunta da mesma fase
      _currentQuestionIndexMundo++;
    } else {
      ...
      if (_pontosMundo >= 9999 * (_currentFaseIndexMundo + 1)) {
        if (_currentFaseIndexMundo < quizFases.length - 1) {
          // Avança para a próxima fase
          _currentFaseIndexMundo++;
  ...
  void _saveGameMundo() {
    _prefsMundo.setInt('currentFaseIndexMundo', _currentFaseIndexMundo);
    _prefsMundo.setInt('pontosMundo', _pontosMundo);
    _prefsMundo.setInt('scoreMundo', _scoreMundo);
  }
            ...
            Stack(
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: Text(
                    (_currentFaseIndexMundo).toString(),
          ...
          children: <Widget>[
            ...
            SizedBox(
              ...
              child: LinearProgressIndicator(
                value: _progressValueMundoAcerto, // Define o valor do progresso
            ...
            SizedBox(
              width: 300,
              child: LinearProgressIndicator(
                value: _progressValueMundoErro, // Define o valor do progresso
                backgroundColor: Colors.grey,
                valueColor: const AlwaysStoppedAnimation<Color>(
            ...
            ...quizFases[_currentFaseIndexMundo]
                ...
                .answers
                .map((answer) {
              final answerIndex = quizFases[_currentFaseIndexMundo]
                  .perguntas[_currentQuestionIndexMundo]
                  .answers
                  .indexOf(answer);
              return SizedBox(
            ...
            Align(
              ...
              child: AnimatedContainer(
                ...
                child: Center(
                    child: Text(
                  ' ${quizFases[_currentFaseIndexMundo].perguntas[_currentQuestionIndexMundo].correctAnswer}',
  ...
  String formatarNumero(int pontosMundo) {
    String numeroFormatado = pontosMundo.toString();
    String resultado = '';
...
//    _currentFaseIndexMundo + 9, // Adicione +1 para exibir o número correto de estrelas
//    (index) => const Image(
//      image: AssetImage('lib/assets/trofeu.png'),
//      width: 100,
//      height: 60,
